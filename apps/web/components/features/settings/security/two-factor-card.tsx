    ...
    <>
      <Card className="backdrop-blur-sm shadow-sm overflow-hidden min-w-0 w-full">
        <CardHeader className="flex flex-row items-center gap-4 space-y-0 w-full min-w-0">
          <div
            className={`p-3 rounded-full shrink-0 ${theme === "destructive" ? "bg-destructive/10" : "bg-primary/10"}`}
          >
            {state.isEnabled ? (
              <ShieldCheck className={`size-6 ${iconThemeClass}`} />
            ) : (
          ...
          {!state.isEnabled && state.hasPassword === false && (
            <div className="space-y-4">
              <div className="p-3 mb-4 rounded-md bg-destructive/10 border border-destructive/20 text-sm text-destructive flex gap-2 items-start font-medium">
                ...
                <p>
                  You must set a local password in the section above before you can enable
                  Two-Factor Authentication.
                </p>
              </div>
              <Button disabled className={buttonThemeClass}>
                Set Up 2FA
              </Button>
            </div>
          )}
          ...
          {!state.isEnabled && state.hasPassword === true && (
            <div className="space-y-4">
              <p className="text-sm text-muted-foreground max-w-2xl">
                ...
                you log in on an unrecognized device.
              </p>
            ...
            <div className="space-y-4 border rounded-md p-4 bg-muted/30">
              <div>
                ...
                <p className="text-xs text-muted-foreground mt-1">
                  Save these codes in a secure place. They can be used to recover your account if
                  you lose your device.
                </p>
              </div>
              <div className="grid grid-cols-2 gap-2 max-w-xs font-mono text-sm">
                {state.backupCodes.map((code, idx) => (
                  <div key={idx} className="bg-background border px-2 py-1 rounded text-center">
                    {code}
                  </div>
                ))}
              </div>
              <Button
                variant="outline"
                size="sm"
                onClick={() => actions.copyToClipboard(state.backupCodes.join("\n"), "codes")}
              >
                {state.copiedCodes ? (
            ...
            <div className="flex flex-col flex-1">
              ...
              <form onSubmit={actions.handlePasswordSubmit} className="flex flex-col flex-1 pt-4">
                ...
                <div className="flex justify-end gap-2 mt-auto pt-6">
                  <Button type="button" variant="ghost" onClick={actions.closeModal}>
                    Cancel
                  </Button>
                  <Button
                    type="submit"
          ...
          {state.step === "method" && (
            <div className="flex flex-col flex-1">
              <DialogHeader>
                ...
                <DialogDescription>
                  Select how you want to receive your security codes.
                </DialogDescription>
              </DialogHeader>
              <div className="flex flex-col gap-3 pt-4 flex-1">
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Use an app like Google Authenticator or Authy to generate codes offline.
                    </p>
                  </div>
                </button>
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-muted-foreground mt-1">
                      Receive a 6-digit verification code directly to your inbox.
                    </p>
                  </div>
                </button>
              </div>
            ...
            <div className="flex flex-col flex-1 relative">
              <div className="absolute top-0 left-0">
                <Button
                  variant="ghost"
                  size="sm"
                  onClick={() => actions.setStep("method")}
              ...
              <DialogHeader>
                ...
                <DialogDescription>
                  Open your preferred TOTP app and scan this code.
                </DialogDescription>
              </DialogHeader>
              <div className="flex flex-col items-center justify-center pt-4 flex-1">
                ...
                <div className="w-full text-center space-y-1 mb-6">
                  <p className="text-xs text-muted-foreground">
                    Can't scan the code? Use this manual entry key:
                  </p>
                  <button
                    onClick={() =>
                      actions.copyToClipboard(actions.extractSecretFromURI(state.qrCodeData), "key")
                    }
                    className="text-xs font-mono bg-muted px-2 py-1 rounded hover:bg-muted/80 transition-colors inline-flex items-center gap-2"
                  >
                ...
                <div className="flex w-full justify-end gap-2 mt-6">
                  ...
                  <Button
                    type="button"
                    onClick={actions.verifyAndEnable}
          ...
          {state.step === "email" && (
            <div className="flex flex-col flex-1 relative">
              ...
              <DialogHeader>
                ...
                <DialogDescription>
                  We just sent a 6-digit code to your email address. Enter it below to enable 2FA.
                </DialogDescription>
              </DialogHeader>
