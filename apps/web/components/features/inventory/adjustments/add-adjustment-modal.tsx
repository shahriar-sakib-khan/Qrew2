...
export function AddAdjustmentModal({ isOpen, onClose, initialProductId }: Props) {
  ...
  const mutation = useMutation({
    mutationFn: async (values: FormValues) => {
      ...
      const signedQty =
        values.adjustmentType === "ADJUSTMENT_OUT" ? (-rawQty).toString() : rawQty.toString();
      ...
      return inventoryApi.createTransaction({
        ...
        notes:
          values.notes ||
          `Stock Adjustment (${values.adjustmentType === "ADJUSTMENT_IN" ? "+IN" : "-OUT"})`,
      });
    },
    onSuccess: () => {
      ...
      toast.success(
        `Stock adjustment (${adjustmentType === "ADJUSTMENT_IN" ? "+IN" : "-OUT"}) recorded.`,
      );
      onClose();
    },
    onError: (err: Error) => {
    ...
    <Dialog open={isOpen} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="sm:max-w-[500px] bg-gray-50 border-gray-200 dark:bg-gray-900/40 dark:border-gray-700">
        <DialogHeader>
          ...
          <DialogDescription>
            Record manual inventory adjustments for stock count corrections or physical audit
            reconciliations.
          </DialogDescription>
        </DialogHeader>
                ...
                <Select value={field.value} onValueChange={field.onChange}>
                  ...
                  <SelectContent>
                    ...
                    <SelectItem value="ADJUSTMENT_OUT">
                      Stock OUT (-) Deduct Correction / Loss
                    </SelectItem>
                  </SelectContent>
                </Select>
              )}
