...
function generateSku(categoryName?: string) {
  const prefix = categoryName ? categoryName.replace(/[^a-zA-Z]/g, "").slice(0, 3).toUpperCase() : "PRD";
...
export function AddProductModal({ isOpen, onClose, editProduct }: Props) {
  ...
  const { control, handleSubmit, reset, setValue, formState: { errors } } = useForm<FormValues>({
    ...
    defaultValues: {
      name: "", unit: "pcs", categoryId: null, brandId: null, sku: null,
      productType: "NORMAL", purchasePrice: null, sellingPrice: null, isActive: true,
    },
  });
  ...
  useEffect(() => {
    ...
    if (editProduct) {
      ...
      reset({
        name: "", unit: "pcs", categoryId: null, brandId: null, sku: generateSku(catObj?.name),
        productType: "NORMAL", purchasePrice: null, sellingPrice: null, isActive: true,
      });
    }
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4 mt-2">
          ...
          <div className="space-y-2">
            ...
            <Controller control={control} name="sku" render={({ field }) => (
              <Input placeholder="Auto-generated or custom SKU" {...field} value={field.value ?? ""} />
            )} />
          </div>
          ...
          <div className="grid grid-cols-2 gap-3">
            <div className="space-y-2">
              ...
              <Controller control={control} name="purchasePrice" render={({ field }) => (
                <Input
                  type="number"
                  step="0.01"
            ...
            <div className="space-y-2">
              ...
              <Controller control={control} name="sellingPrice" render={({ field }) => (
                <Input
                  type="number"
                  step="0.01"
                  onWheel={(e) => (e.target as HTMLElement).blur()}
