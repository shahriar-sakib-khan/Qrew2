...
export default function SignInPage() {
  ...
  const onSubmit = async (data: SignInValues) => {
    ...
    // 1. Capture the 'data' payload which contains the user's role
    ...
    // 2. Extract role and hard-redirect to the correct layout
      ...
      <div className="flex-1 grid place-items-center px-4 py-12 z-10">
        <Card className="card-animate w-full max-w-sm border-border/50 bg-background/80 backdrop-blur-xl shadow-2xl">
          ...
          <CardContent className="grid gap-5">
            ...
            <Button variant="outline" className="w-full h-10 bg-background/50" onClick={handleGoogleSignIn} disabled={isFormDisabled}>
              {loadingType === "google" ? (
          ...
          <CardFooter className="flex justify-center border-t border-border/50 pt-6 text-sm text-muted-foreground">
            ...
            <Link href="/sign-up" className="ml-1 text-foreground font-medium hover:underline">
              Create one
            </Link>
          </CardFooter>
        </Card>
