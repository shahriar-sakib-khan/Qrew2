...
export class InventoryController {
  ...
  static async getTransactions(c: Context) {
    ...
    const result = await db.query.inventoryTransactions.findMany({
      ...
      with: {
        product: { columns: { id: true, name: true, unit: true, purchasePrice: true, sellingPrice: true } },
        ...
        db.query.purchases.findMany({
          where: (t, { inArray, eq, or, and }) =>
            and(
              ...
              or(
                inArray(t.id, candidateRefIds),
                inArray(t.purchaseNumber, candidateRefIds)
              )
            ),
          with: {
        ...
        db.query.sales.findMany({
          where: (t, { inArray, eq, or, and }) =>
            and(
              ...
              or(
                inArray(t.id, candidateRefIds),
                inArray(t.saleNumber, candidateRefIds)
              )
            ),
          with: {
        ...
        db.query.saleReturns.findMany({
          where: (t, { inArray, eq, or, and }) =>
            and(
              ...
              or(
                inArray(t.id, candidateRefIds),
                inArray(t.returnNumber, candidateRefIds)
              )
            ),
          with: {
            customer: { columns: { id: true, name: true } },
  ...
  static async getEligibleReturns(c: Context) {
    try {
      ...
      if (type === 'SALE_RETURN') {
        ...
        let existingReturnsMap: Record<string, number> = {};
        if (refIds.length > 0) {
        ...
        salesList.forEach(sale => {
          sale.items.forEach(item => {
            ...
            const prevReturned = (existingReturnsMap[keyByUuid] || 0) + (existingReturnsMap[keyByNum] || 0);
    ...
    try {
      ...
      const singleTxSchema = z.object({
        ...
        quantity: z.union([z.string(), z.number()])
          ...
          .refine((val) => !isNaN(parseFloat(val)) && parseFloat(val) !== 0, 'Valid quantity required'),
        transactionType: z.preprocess(
          (val) => (!val || val === '' ? '' : String(val).toUpperCase()),
          z.enum([
            'PURCHASE', 'PURCHASE_RETURN', 'SALE', 'SALE_RETURN',
            'REFILL_IN', 'ADJUSTMENT_IN', 'ADJUSTMENT_OUT'
          ])
        ),
      ...
      if (body && Array.isArray(body.items)) {
        ...
        if (!parsedMulti.success) {
          console.error('Multi transaction validation failed:', JSON.stringify(parsedMulti.error.format()));
          const issues = parsedMulti.error.issues.map(i => `${i.path.join('.')}: ${i.message}`).join('; ');
          return c.json({ error: `Validation Error: ${issues}`, details: parsedMulti.error.format() }, 400);
        }
        ...
        if (!parsed.success) {
          console.error('Single transaction validation failed:', JSON.stringify(parsed.error.format()));
      ...
      for (const item of itemsToValidate) {
        const isReturn = item.transactionType === 'SALE_RETURN' || item.transactionType === 'PURCHASE_RETURN';
        if (isReturn) {
          if (!item.referenceId || !item.referenceId.trim()) {
            return c.json({ error: `Validation Error: Every return item MUST be linked to an original sale invoice or purchase bill (referenceId required).` }, 400);
          }
          ...
          if (item.transactionType === 'SALE_RETURN') {
            ...
            if (!sale) {
              return c.json({ error: `Validation Error: Original sale invoice '${refId}' was not found or is not confirmed.` }, 400);
            }

            const matchingItem = sale.items.find((it: any) => it.productId === item.productId && it.stockState === item.stockState);
            if (!matchingItem) {
              return c.json({ error: `Validation Error: Selected product is not present in Sale Invoice #${sale.saleNumber}.` }, 400);
            }
            ...
            const prevReturnedQty = prevTxRows.reduce((sum, txRow) => sum + Math.abs(parseFloat(txRow.quantity || '0')), 0);
            ...
            if (!purchase) {
              return c.json({ error: `Validation Error: Original purchase bill '${refId}' was not found or is not confirmed.` }, 400);
            }

            const matchingItem = purchase.items.find((it: any) => it.productId === item.productId && it.stockState === item.stockState);
            if (!matchingItem) {
              return c.json({ error: `Validation Error: Selected product is not present in Purchase Bill #${purchase.purchaseNumber}.` }, 400);
            }

            const refIdsToQuery = Array.from(new Set([purchase.id, purchase.purchaseNumber].filter(Boolean)));
            const prevTxRows = await db.query.inventoryTransactions.findMany({
              where: (t, { eq, and, inArray }) => and(
                eq(t.organizationId, orgId),
                eq(t.referenceType, 'PURCHASE'),
      ...
      const refId = parsedSingleData.referenceId?.trim() ? parsedSingleData.referenceId.trim() : uuidv4();
