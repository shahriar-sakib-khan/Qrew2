...
describe("updateCharge", () => {
  ...
  it("returns 422 when updated formula has invalid syntax", async () => {
    ...
    const ctx = makeCtx({ params: { chargeId: CHARGE_ID }, body: { formula: "PORT_DUES /" } });
    const res = await updateCharge(ctx);
  ...
  it("returns 409 when new chargeToken collides with sibling charge", async () => {
    ...
    (db.query.templateRowCharges.findFirst as any).mockResolvedValue(makeRowCharge({ id: "other-chg", chargeToken: "PORT_DUES_VAT" }));
    const ctx = makeCtx({ params: { chargeId: CHARGE_ID }, body: { chargeToken: "PORT_DUES_VAT" } });
    const res = await updateCharge(ctx);
  ...
  it("updates formula only — response is decoded", async () => {
    mockChargeOwned(makeRowCharge(), true);
    const updated = makeRowCharge({ formula: "PORT_DUES * 0.20" });
    mockUpdateReturns(updated);
    const ctx = makeCtx({ params: { chargeId: CHARGE_ID }, body: { formula: "PORT_DUES * 0.20" } });
    const res = await updateCharge(ctx);
    expect(res.status).toBe(200);
    expect((res as any).data.formula).toBe("PORT_DUES * 0.20");
  });
  ...
  it("updates chargeToken only — label is NOT automatically changed (AGENTS.md rule)", async () => {
    ...
    mockUpdateReturns(updated);
    const ctx = makeCtx({ params: { chargeId: CHARGE_ID }, body: { chargeToken: "PORT_DUES_TAX" } });
    const res = await updateCharge(ctx);
    expect(res.status).toBe(200);
