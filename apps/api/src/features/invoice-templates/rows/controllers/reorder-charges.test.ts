import { reorderCharges } from "./manage-row-charge.controller";
/**
...
describe("TemplateRowChargesController - reorderCharges", () => {
  beforeEach(() => {
    vi.resetAllMocks();
    (db.transaction as any).mockImplementation(async (fn: any) => fn(db));
  });

describe("reorderCharges", () => {
  it("returns 401 when unauthenticated", async () => {
    const ctx = makeCtx({ orgId: null, params: { rowId: ROW_ID }, body: { orderedIds: [CHARGE_ID] } });
    const res = await reorderCharges(ctx);
    expect(res.status).toBe(401);
  });

  it("returns 404 when row not found or wrong org", async () => {
    mockRowNotFound();
    const ctx = makeCtx({ params: { rowId: ROW_ID }, body: { orderedIds: [CHARGE_ID] } });
    const res = await reorderCharges(ctx);
    expect(res.status).toBe(404);
  });

  it("returns 400 when orderedIds contains non-UUID values", async () => {
    (db.select as any).mockReturnValueOnce(hoistedChain([{ id: ROW_ID }]));
    const ctx = makeCtx({ params: { rowId: ROW_ID }, body: { orderedIds: ["not-a-uuid"] } });
    const res = await reorderCharges(ctx);
