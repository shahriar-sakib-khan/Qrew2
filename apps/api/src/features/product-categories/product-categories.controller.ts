...
export class ProductCategoriesController {
  ...
  static async update(c: Context) {
    ...
    const [updated] = await db.update(productCategories)
      .set({ name: parsed.data.name, description: parsed.data.description, isActive: parsed.data.isActive })
  ...
  static async remove(c: Context) {
    ...
    // Guard: DB FK (products.categoryId → product_categories.id ON DELETE SET NULL) handles orphan removal.
