...
describe("Invoice Engine Engine & Security Hardening Unit Tests", () => {
  ...
  describe("DagValidatorService Strict Restrictions", () => {
    ...
    it("rejects section charges with formulaBase !== BASE", () => {
      const sections: EvaluatorSection[] = [
        {
          id: "sec-1",
          sectionToken: "SEC1",
          ...
          sectionCharges: [
            {
              id: "sc-1",
              chargeToken: "SEC_SEC1_TAX",
              label: "Tax",
              formulaBase: "TOTAL" as any, // ILLEGAL: must be BASE
              formulaRest: "* 0.05",
              sortOrder: 0,
            },
          ],
    ...
    it("rejects row bases referencing forward tokens", () => {
      const sections: EvaluatorSection[] = [
        {
          id: "sec-1",
          sectionToken: "SEC1",
          ...
          rows: [
            {
              ...
              sectionId: "sec-1",
              valueType: "formula",
              formula: "ROW_2 * 2", // ILLEGAL: referencing ROW_2 which is defined after
              sortOrder: 0,
              charges: [],
            },
            {
      ...
      expect(result.errors[0].message).toContain('references "ROW_2" which has not been defined yet');
    });
  });
  ...
  describe("AstEvaluatorService", () => {
    it("evaluates simple base values and row charges accurately using BigNumber precision", () => {
      ...
      const { evaluatedSections, grandTotal, errors } = AstEvaluatorService.evaluate(sections);
