...
import { z } from 'zod';
import {
  db, purchases, purchaseItems, inventoryTransactions, products,
} from '@starter/db';
...
export class PurchasesController {
  ...
  static async create(c: Context) {
    ...
    await db.transaction(async (tx) => {
      ...
      [createdPurchase] = await tx.insert(purchases).values({
        id: purchaseId,
  ...
  static async remove(c: Context) {
    ...
    // purchase_items are cascade-deleted by the FK.
  ...
  static async confirm(c: Context) {
    ...
    await db.transaction(async (tx) => {
      ...
      await tx.update(purchases).set({ status: 'CONFIRMED' }).where(eq(purchases.id, id));
    });
