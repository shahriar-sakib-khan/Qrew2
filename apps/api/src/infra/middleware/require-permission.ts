...
export const requireOrgPermission = (requiredPermission: string) => {
  return async (c: Context, next: Next) => {
    ...
    const currentMember = await db.query.members.findFirst({
      where: and(eq(members.userId, sessionData.user.id), eq(members.organizationId, orgId)),
    });
    ...
    if (!userPermissions.has(requiredPermission)) {
      console.warn(
        `[PBAC] Blocked: User ${sessionData.user.id} attempted to access ${requiredPermission}`,
      );
      return c.json(
        {
          error: "Forbidden",
